...
inp_num_but.addEventListener("click", function(){
  if(GL_login == true){
    ...
    innumer.innerText = browser.i18n.getMessage( "first_logintest" ) //"Zuerst erfolgreichen <br>Logintest durchführen"
  }
});
...
function init(x){
  ...
  if(x == "start"){
    ...
    browser.storage.local.get("option")
    .then(init, onError);
  }
