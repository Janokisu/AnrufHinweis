...
.then((result) => {
  for(permission of result.permissions){
    if(permission == "notifications"){
      GL_permission_note = true;
    }
    else if(permission == "nativeMessaging"){
      GL_permission_nati = true;
    }
  }
  
  for(origin of result.origins){
    if(origin == "http://*/*" || origin == "http://*/*"){
      GL_permission_webs = true;
    }
  }
});
...
function init(x){
  ...
  if(x == "start"){
    browser.storage.local.get(["option","modi"])
    .then(init, onError);
  }
  else{
    //console.log("test", x);
    if( x.hasOwnProperty("option") ){
      ...
      if( opt.hasOwnProperty("FUrl") ){
        GL_Fritz_URL= opt["FUrl"];
      }
      
      if( opt.hasOwnProperty("num_reg") ){
        GL_check_num_reg  = opt["num_reg"];
      }
      
      if( opt.hasOwnProperty("num_anz") ){
        GL_check_num_anz  = opt["num_anz"];
      }
      
      if( opt.hasOwnProperty("anum_anz") ){
        GL_check_anum_anz = opt["anum_anz"];
      }
      ...
      if( opt.hasOwnProperty("alarm_vol") ){
        GL_alarm.volume = Number(opt["alarm_vol"]);
      }
      
      if( opt.hasOwnProperty("verpa_anz") ){
        GL_verpa_anz = opt["verpa_anz"];
      }
    ...
    if( x.hasOwnProperty("modi") ){
      GL_modi = Number(x["modi"]);
    }
    else{
      GL_modi = -1;
    }
  ...
  switch(info[2]){
    ...
    case "RING": 
      ...
      */
      ...
      if(GL_check_num_reg == true && GL_only_num_reg[ WORK_anummer[ info[3] ] ] != true){
        WORK_anummer[ info[3] ] = "";
      }
      ...
      if(WORK_anummer[ info[3] ] != ""){
        ...
        let wer = info[4];
        if(wer == ""){
          wer = browser.i18n.getMessage("caller_verb"); //Nr. verborgen
        }
        ...
        if(GL_check_anum_anz == true){
          extra = browser.i18n.getMessage("ring_extra", WORK_anummer[ info[3] ]);
        }
    ...
    case "CONNECT": 
      ...
      */
      ...
      if(WORK_anummer[ info[3] ] != ""){
        ...
        //if(GL_check_anum_anz == true) extra = " auf der Nummer " + WORK_anummer[ info[3] ];
        if(GL_check_anum_anz == true){
          extra = browser.i18n.getMessage("connect_extra", WORK_anummer[ info[3] ]);
        }
    ...
    case "DISCONNECT": 
      ...
      */
      ...
      if(WORK_anummer[ info[3] ] != ""){
        ...
        //if(GL_check_anum_anz == true) extra = " auf der Nummer " + WORK_anummer[ info[3] ];
        if(GL_check_anum_anz == true){
          extra = browser.i18n.getMessage("disconnect_extra", WORK_anummer[ info[3] ]);
        }
        WORK_anummer[ info[3] ] = "";
...
function blink_connecting(x, out_in){
  ...
  if(blink_connecting_toggle){
    if(out_in == 1){
      ...
      browser.browserAction.setBadgeBackgroundColor({color: "#0000CC"});
    }
...
function PhonebookList(lua_URL){
  XHttpRequest(lua_URL)
  .then(prepareViewPhonebookList, onError);
}
...
function prepareViewPhonebookList(request){
  ...
  for(call of calls){
    ...
    for(number of call.getElementsByTagName("number")){
      ...
      if(tmp == "" || tmp == "intern" || tmp == "memo"){
        continue; //interne Nummern überspringen
      }
    ...
    if(GL_phone_book_list2[name].phonenumber.length == 0){
      delete GL_phone_book_list2[name];
    }
  }
...
function pop(){
  ...
  for(call of GL_call_list){
    ...
    if(GL_check_num_anz == true && GL_only_num_reg[ call["CallederNumber"] ] != true){
      continue;
    }
    ...
    if(num == ""){
      num = browser.i18n.getMessage("caller_verb"); //Nr. verborgen
    }
    else{
...
function calllist(lua_URL){
  XHttpRequest(lua_URL)
  .then(prepareViewCallList, onError);
}
...
function prepareViewCallList(request){
  ...
  for(call of calls){
    ...
    let CallederNumber = "";
    if(call.getElementsByTagName("CalledNumber").length > 0){
      CallederNumber = call.getElementsByTagName("CalledNumber")[0].textContent;
    }
    else if(call.getElementsByTagName("CallerNumber").length > 0){
      CallederNumber = call.getElementsByTagName("CallerNumber")[0].textContent;
    }
